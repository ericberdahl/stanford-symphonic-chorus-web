...
const helpers = {
    ...
    renderTemplate: (handlebars, context, templateString, options) => {
    ...
    sentence: (handlebars, context, options) => {
        ...
        const renderer = _.defaultTo(options.fn, (i) => i);
...
const dateHelpers = {
    formatDate: (handlebars, format, date, options) => {
        const optionsHash = _.defaults(options.hash, {
            tz: defaultTimezone
        });

        date = moment(date);
        if (optionsHash.tz) {
            date = date.tz(optionsHash.tz);
        }
